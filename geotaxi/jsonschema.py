    ...
    if data_is_dict:
        ...
        if "operator" in data_keys:
            ...
            if not isinstance(data__operator, (str)):
                raise JsonSchemaException("data.operator must be string", value=data__operator, name="data.operator", definition={'type': 'string'}, rule='type')
        if "lat" in data_keys:
            data_keys.remove("lat")
            data__lat = data["lat"]
            if not isinstance(data__lat, (int, float, str)) or isinstance(data__lat, bool):
                raise JsonSchemaException("data.lat must be number or string", value=data__lat, name="data.lat", definition={'type': ['number', 'string']}, rule='type')
        if "device" in data_keys:
            data_keys.remove("device")
            data__device = data["device"]
            if not isinstance(data__device, (str)):
                raise JsonSchemaException("data.device must be string", value=data__device, name="data.device", definition={'type': 'string'}, rule='type')
        if "lon" in data_keys:
            data_keys.remove("lon")
            data__lon = data["lon"]
            if not isinstance(data__lon, (int, float, str)) or isinstance(data__lon, bool):
                raise JsonSchemaException("data.lon must be number or string", value=data__lon, name="data.lon", definition={'type': ['number', 'string']}, rule='type')
        if "timestamp" in data_keys:
            ...
            if not isinstance(data__timestamp, (int, float, str)) or isinstance(data__timestamp, bool):
                raise JsonSchemaException("data.timestamp must be number or string", value=data__timestamp, name="data.timestamp", definition={'type': ['number', 'string']}, rule='type')
        if "status" in data_keys:
            data_keys.remove("status")
            data__status = data["status"]
            if not isinstance(data__status, (str)):
                raise JsonSchemaException("data.status must be string", value=data__status, name="data.status", definition={'type': 'string'}, rule='type')
        ...
        if "taxi" in data_keys:
            ...
            if not isinstance(data__taxi, (str)):
                raise JsonSchemaException("data.taxi must be string", value=data__taxi, name="data.taxi", definition={'type': 'string'}, rule='type')
        if "hash" in data_keys:
            data_keys.remove("hash")
            data__hash = data["hash"]
            if not isinstance(data__hash, (str)):
                raise JsonSchemaException("data.hash must be string", value=data__hash, name="data.hash", definition={'type': 'string'}, rule='type')
    return data